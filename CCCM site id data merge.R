...
## Load cleaned data
response <- read.xlsx("./output/external/CCCM_SiteReporting_Week 1 Cleaned external_2019-12-09.xlsx")
#response <- cleanHead(response)
#response <- cleanHead(response)


## Step 1: Data Merge - dots
lookup <- data.frame(colour = c("not_applicable", "non_existent", "inadequate", "adequate"), path = c("./merge/grey.png", "./merge/red.png", "./merge/orange.png",   
                                                                            "./merge/green.png"))
...
data_rename <- response %>% select("b4_site_smc_agency_name", "c1_type_of_site", "c2_landowner",
